...
public class CreateJobPanel extends JPanel {
    ...
    private void styleInputs() {
        ...
        for (JTextField f : new JTextField[]{
                ...
                departmentField, instructorNameField, instructorEmailField, deadlineField
        }) {
            f.setFont(fieldFont);
    ...
    private JPanel buildDetailedSection() {
        ...
        skillPoolField.setEditable(false);
        skillPoolField.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
        grid.add(skillPoolField, g);
        row++;

        JPanel skillControls = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        skillControls.setOpaque(false);
        skillControls.add(hintLabel("Select a skill from the pool and add it to the job"));
        MoUiTheme.styleOutlineButton(addSkillButton, 8);
        addSkillButton.addActionListener(e -> addSkillFromPool());
        skillControls.add(addSkillButton);

        g.gridx = 1;
        g.gridy = row;
        g.gridwidth = 2;
        g.insets = new Insets(0, 0, 8, 0);
        grid.add(skillControls, g);
        row++;
    ...
    private void addSkillFromPool() {
        String s = skillPool.isEmpty() ? null : skillPool.get(0);
        if (s == null || s.isBlank()) {
            return;
        }
    ...
    private void loadSkillPool() {
        skillPool.clear();
        skillPool.add("Python");
        skillPool.add("Java");
        skillPool.add("Git/GitHub");
        skillPool.add("Teaching Experience");
        skillPool.add("C++");
        skillPool.add("LaTeX");
        skillPool.add("Algorithm Design");
        skillPool.add("Tutoring experience");
        skillPool.add("Experience with online learning platforms");
        skillPoolField.setText(String.join(", ", skillPool));
    }
    ...
    private void populateFromJob(Job j) {
        ...
        if (j.getRequiredSkills() != null) {
            for (String skill : j.getRequiredSkills()) {
                if (skillPool.isEmpty() || skillPool.contains(skill)) {
                    skills.add(skill);
                }
            }
        }
