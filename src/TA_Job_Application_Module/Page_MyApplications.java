...
public class Page_MyApplications {
    ...
    /** 草稿表第 2–7 列固定宽度；JOB TITLE 列用剩余宽度铺满视口 */
    ...
    /** 切换到「草稿」标签页（标题可能为 Drafts 或 Drafts (n)） */
    ...
    private void buildTabbedPane() {
        ...
        applicationsTab.add(new JScrollPane(applicationsTable), BorderLayout.CENTER);

        JPanel draftsTab = new JPanel(new BorderLayout());
        draftsTab.setBackground(UI_Constants.BG_COLOR);
        buildDraftsTable();
        draftsTab.add(draftsTable.getTableHeader(), BorderLayout.NORTH);
    ...
    /** 草稿表：固定列总宽 + JOB TITLE 填满视口，消除右侧空白 */
    private void syncDraftJobTitleColumnToFillViewport() {
        ...
        int spacing = draftsTable.getIntercellSpacing().width * 6;
        int fudge = 8;
        int titleW = vw - fixed - spacing - fudge;
        titleW = Math.max(titleW, JOB_TITLE_COL_MIN_WIDTH);
