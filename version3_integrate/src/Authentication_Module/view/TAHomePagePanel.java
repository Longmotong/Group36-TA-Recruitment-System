...
public class TAHomePagePanel extends JPanel {
    ...
    private ProfileData buildProfileFromAuthUser(User user, TAUser taUser) {
        ...
        if (taUser != null) {
            ...
            if (taUser.getSkills() != null) {
                ...
                // Prefer the new taSkillPool structure so all selected skills survive restart/login.
                if (taUser.getSkills().getTaSkillPool() != null) {
                    TAUser.TaSkillPool pool = taUser.getSkills().getTaSkillPool();
                    if (pool.getTechnicalSkills() != null) {
                        copySelectedSkillItems(
                                p.skills,
                                pool.getTechnicalSkills().getProgrammingAndSoftwareFundamentals(),
                                "Programming Languages"
                        );
                        copySelectedSkillItems(
                                p.skills,
                                pool.getTechnicalSkills().getHardwareAndLogicDesign(),
                                "Hardware & Logic Design"
                        );
                        copySelectedSkillItems(
                                p.skills,
                                pool.getTechnicalSkills().getEmbeddedSystemsAndLowLevelDevelopment(),
                                "Embedded Systems"
                        );
                    }
                    if (pool.getEngineeringAndTools() != null) {
                        copySelectedSkillItems(
                                p.skills,
                                pool.getEngineeringAndTools().getProfessionalDevelopmentAndSimulationTools(),
                                "Professional Development & Simulation Tools"
                        );
                    }
                    if (pool.getLanguageAndCommunication() != null) {
                        copySelectedSkillItems(
                                p.skills,
                                pool.getLanguageAndCommunication().getCrossCulturalCommunication(),
                                "English"
                        );
                    }
                } else {
    ...
    private void copySelectedSkillItems(
            java.util.List<profile_module.data.SkillItem> target,
            java.util.List<TAUser.SkillItem> source,
            String category
    ) {
        if (source == null) return;
        for (TAUser.SkillItem s : source) {
            if (s == null || !s.isSelected()) continue;
            profile_module.data.SkillItem item = new profile_module.data.SkillItem();
            item.name = s.getName();
