...
public final class OnboardingScreen extends JPanel {
    ...
    // Step 2 skills（与 EditSkillsScreen 一致：名称 + 类型 + 掌握程度）
    ...
    /**
     * 刷新步骤1的文本框：不清空 placeholder，用户重新填写。
     */
    private void refreshStep1Fields() {
        // 不预填已有数据，保持 placeholder 提示
    }
    ...
    private void persistStep1() {
        ...
        app.updateProfile(p);
        // 同步到 TAUser 并保存到 JSON 文件
        syncProfileToTAUser(p);
    }

    /**
     * 结束引导流程：刷新完成度、标记 onboarding 已完成并持久化，再进入仪表盘。
     */
    ...
    /**
     * 将 ProfileData 的 profile 字段同步到 TAUser 并保存到 JSON 文件。
     */
    ...
    /**
     * 将 ProfileData.skills 同步到 TAUser.skills，并保存到 JSON 文件。
     */
