...
public final class OnboardingScreen extends JPanel {
    ...
    /** Phone: exactly 10 digits, no spaces or punctuation (e.g. 5551234567). */
    ...
    private final JTextField phone = Ui.textField("e.g. 5551234567 (10 digits)");
    ...
    private JPanel buildStep3Card() {
        ...
        JPanel right = new JPanel(new FlowLayout(FlowLayout.RIGHT, 12, 0));
        right.setOpaque(false);
        step3SkipButton.addActionListener(e -> {
            persistStep1();
    ...
    /**
     * @return null if OK, otherwise English message for JOptionPane
     */
    private String validateStep1() {
        ...
        if (!PHONE_PATTERN.matcher(ph).matches()) {
            return "Please enter a valid 10-digit phone number (digits only, no spaces or dashes).";
        }
        return null;
    }
    ...
    private void refreshSkillsEmptyState() {
        ...
        if (!empty) {
            for (SkillItem item : p.skills) {
                ...
                skillsPreviewPanel.add(Chip.blue(item.name.trim(), false));
            }
        }
        revalidate();
        repaint();
    ...
    /** CV is "filled" after Browse sets a non-empty {@code CvInfo.fileName}. */
    private void refreshStep3SkipState() {
        ProfileData p = app.profile();
