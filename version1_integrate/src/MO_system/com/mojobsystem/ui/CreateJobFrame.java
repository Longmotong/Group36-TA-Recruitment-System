...
public class CreateJobFrame extends JFrame {
    ...
    private NavigationPanel.Actions navActions() {
        return new NavigationPanel.Actions(
                () -> MoFrameGeometry.navigateReplace(this, () -> new IntegratedDashboardFrame().setVisible(true)),
    ...
    private void submitForm() {
        ...
        List<Job> moJobs = new ArrayList<>(jobRepository.loadJobsForMo(com.mojobsystem.MoContext.CURRENT_MO_ID));
        if (editingJob != null) {
            Job job = new Job();
            ...
            for (int i = 0; i < moJobs.size(); i++) {
                if (job.getId().equals(moJobs.get(i).getId())) {
                    moJobs.set(i, job);
                    replaced = true;
                    break;
                }
            ...
            jobRepository.saveJobsForMo(com.mojobsystem.MoContext.CURRENT_MO_ID, moJobs);
        } else {
