...
public class DataService {
    ...
    static {
        ...
        if (dataPath != null && !dataPath.isEmpty()) {
            ...
            DATA_ROOT = AppDataRoot.asFile().getAbsolutePath();
        }
    ...
    /** 当前 TA 用户 JSON 所在目录（加载成功后确定，保存时写回同目录） */
    private File usersTaDirectory;
    
    /**
     * 获取Jobs文件夹的绝对路径，尝试多个可能的位置
     */
    private File findJobsDirectory() {
        File fromConfiguredRoot = new File(resolveDataRootDirectory(), "jobs");
        if (fromConfiguredRoot.exists() && fromConfiguredRoot.isDirectory()) {
            File[] jf = fromConfiguredRoot.listFiles((d, name) -> name.endsWith(".json"));
            int fileCount = jf != null ? jf.length : 0;
            System.out.println("Found jobs directory [data root]: " + fromConfiguredRoot.getAbsolutePath()
                    + " (" + fileCount + " JSON files)");
            return fromConfiguredRoot;
        }
        // 尝试的路径列表（按优先级从高到低）
    ...
    private File findApplicationsDirectory() {
        File fromConfiguredRoot = new File(resolveDataRootDirectory(), "applications");
        if (fromConfiguredRoot.exists() && fromConfiguredRoot.isDirectory()) {
            File[] files = fromConfiguredRoot.listFiles((d, name) -> name.endsWith(".json"));
            int fileCount = files != null ? files.length : 0;
            System.out.println("Found applications directory [data root]: " + fromConfiguredRoot.getAbsolutePath()
                    + " (" + fileCount + " JSON files)");
            return fromConfiguredRoot;
        }
        String userDir = System.getProperty("user.dir");
    ...
    /**
     * 解析 data/users/ta 目录（与职位、申请目录探测策略一致）
     */
    private File findUsersTaDirectory() {
        File fromConfiguredRoot = new File(resolveDataRootDirectory(), "users" + File.separator + "ta");
        if (fromConfiguredRoot.exists() && fromConfiguredRoot.isDirectory()) {
            File[] files = fromConfiguredRoot.listFiles((d, name) -> name.endsWith(".json"));
            int n = files != null ? files.length : 0;
            System.out.println("Found users/ta directory [data root]: " + fromConfiguredRoot.getAbsolutePath()
                    + " (" + n + " JSON)");
            return fromConfiguredRoot;
        }
        String userDir = System.getProperty("user.dir");
        String[][] possiblePaths = {
            { userDir + File.separator + "data" + File.separator + "users" + File.separator + "ta", "cwd/data/users/ta" },
            { userDir + File.separator + ".." + File.separator + "data" + File.separator + "users" + File.separator + "ta", "version1/../data/users/ta" },
            { userDir + File.separator + ".." + File.separator + ".." + File.separator + "data" + File.separator + "users" + File.separator + "ta", "nested/../data/users/ta" },
            { userDir + File.separator + "src" + File.separator + "data" + File.separator + "users" + File.separator + "ta", "cwd/src/data/users/ta" },
            { ".." + File.separator + "data" + File.separator + "users" + File.separator + "ta", "relative ../data/users/ta" },
            { DATA_ROOT + File.separator + "users" + File.separator + "ta", "DATA_ROOT/users/ta" }
        };
        for (String[] pathInfo : possiblePaths) {
            ...
            if (dir.exists() && dir.isDirectory()) {
                File[] files = dir.listFiles((d, name) -> name.endsWith(".json"));
                int n = files != null ? files.length : 0;
                System.out.println("Found users/ta directory [" + pathInfo[1] + "]: " + dir.getAbsolutePath() + " (" + n + " JSON)");
                return dir;
            }
        }
        File fallback = new File(DATA_ROOT + File.separator + "users" + File.separator + "ta");
        System.out.println("Using default users/ta path: " + fallback.getAbsolutePath());
        return fallback;
    }
    ...
    /** Clears singleton so the next {@link #getInstance()} reloads jobs/apps/user for a new login session. */
    public static void resetInstance() {
        instance = null;
    }

    /**
     * Root {@code data} directory: {@code taportal.data.path} if set, otherwise {@link #DATA_ROOT}.
     */
    public static File resolveDataRootDirectory() {
        String dataPath = System.getProperty("taportal.data.path");
        if (dataPath != null && !dataPath.trim().isEmpty()) {
            return new File(dataPath.trim());
        }
    ...
    private void loadCurrentUser() {
        usersTaDirectory = findUsersTaDirectory();
        String loginName = System.getProperty("taportal.login.username");
        if (usersTaDirectory.exists() && loginName != null && !loginName.isBlank()) {
            File[] userFiles = usersTaDirectory.listFiles((d, name) -> name.endsWith(".json") && name.startsWith("user_ta_"));
            if (userFiles != null && userFiles.length > 0) {
                Arrays.sort(userFiles, Comparator.comparing(File::getName));
                for (File uf : userFiles) {
                    try (Reader reader = new FileReader(uf)) {
                        TAUser loaded = gson.fromJson(reader, TAUser.class);
                        if (loaded != null && loaded.getAccount() != null
                                && loginName.trim().equalsIgnoreCase(loaded.getAccount().getUsername())
                                && loaded.getProfile() != null && loaded.getProfile().getStudentId() != null
                                && !loaded.getProfile().getStudentId().isBlank()) {
                            currentUser = loaded;
                            ensureUserDefaults();
                            recomputeProfileCompletion();
                            System.out.println("Loaded TA user for session from: " + uf.getAbsolutePath());
                            return;
                        }
                    } catch (IOException e) {
                        System.err.println("Error reading user file " + uf.getName() + ": " + e.getMessage());
                    }
                }
            }
        }
        if (usersTaDirectory.exists()) {
            File[] userFiles = usersTaDirectory.listFiles((d, name) -> name.endsWith(".json") && name.startsWith("user_ta_"));
            if (userFiles != null && userFiles.length > 0) {
                Arrays.sort(userFiles, Comparator.comparing(File::getName));
                for (File uf : userFiles) {
                    try (Reader reader = new FileReader(uf)) {
                        TAUser loaded = gson.fromJson(reader, TAUser.class);
                        if (loaded != null && loaded.getProfile() != null && loaded.getProfile().getStudentId() != null
                                && !loaded.getProfile().getStudentId().isBlank()) {
                            currentUser = loaded;
                            ensureUserDefaults();
                            recomputeProfileCompletion();
                            System.out.println("Loaded TA user from: " + uf.getAbsolutePath());
                            return;
                        }
                    } catch (IOException e) {
                        System.err.println("Error reading user file " + uf.getName() + ": " + e.getMessage());
        ...
        loadMockCurrentUser();
        if (!usersTaDirectory.exists()) {
            usersTaDirectory.mkdirs();
        }
        saveCurrentUser();
    }

    private void loadMockCurrentUser() {
    ...
    private void ensureUserDefaults() {
        if (currentUser.getApplicationSummary() == null) {
            currentUser.setApplicationSummary(new TAUser.ApplicationSummary());
        }
        if (currentUser.getCv() == null) {
            currentUser.setCv(new TAUser.CV());
        }
        if (currentUser.getSkills() == null) {
            ...
            currentUser.setSkills(s);
        }
        if (currentUser.getAcademic() == null) {
            currentUser.setAcademic(new TAUser.Academic());
        }
        if (currentUser.getAccount() == null) {
            currentUser.setAccount(new TAUser.Account());
        }
    }

    /**
     * 将当前用户写回 data/users/ta/user_ta_{studentId}.json（与仓库中示例结构一致）
     */
    public void saveCurrentUser() {
        if (currentUser == null || currentUser.getProfile() == null) {
            return;
        }
        String sid = currentUser.getProfile().getStudentId();
        if (sid == null || sid.isBlank()) {
            sid = "unknown";
        }
        try {
            if (usersTaDirectory == null) {
                usersTaDirectory = findUsersTaDirectory();
            }
            if (!usersTaDirectory.exists()) {
                usersTaDirectory.mkdirs();
            }
            String fileName = "user_ta_" + sid + ".json";
            File out = new File(usersTaDirectory, fileName);
            Gson prettyGson = new GsonBuilder().setPrettyPrinting().create();
            String json = prettyGson.toJson(currentUser);
            java.nio.file.Files.writeString(out.toPath(), json);
            System.out.println("User saved to: " + out.getAbsolutePath());
        } catch (IOException e) {
    ...
    /**
     * 与 profile 模块一致的完成度规则，写回 {@link TAUser#profileCompletion}
     */
    public void recomputeProfileCompletion() {
        if (currentUser == null || currentUser.getProfile() == null) {
            return;
        }
        TAUser.Profile p = currentUser.getProfile();
        int total = 6;
        int filled = 0;
        if (!blank(p.getFullName())) filled++;
        if (!blank(p.getStudentId())) filled++;
        if (!blank(p.getYear())) filled++;
        if (!blank(p.getProgramMajor())) filled++;
        if (currentUser.getAccount() != null && !blank(currentUser.getAccount().getEmail())) filled++;
        if (!blank(p.getPhoneNumber())) filled++;
        int base = (int) Math.round(100.0 * filled / total);
        int bonus = 0;
        TAUser.Skills sk = currentUser.getSkills();
        if (skillsNonEmpty(sk)) {
            bonus += 10;
        }
        TAUser.CV cv = currentUser.getCv();
        if (cv != null && cv.isUploaded() && !blank(cv.getOriginalFileName())) {
            bonus += 10;
        }
        currentUser.setProfileCompletion(Math.min(100, base + bonus));
    }
    ...
    private static boolean skillsNonEmpty(TAUser.Skills sk) {
        if (sk == null) return false;
        return nonEmptyList(sk.getProgramming()) || nonEmptyList(sk.getTeaching())
                || nonEmptyList(sk.getCommunication()) || nonEmptyList(sk.getOther());
    }
    ...
    public void syncApplicationSummaryFromApplications() {
        if (currentUser == null) {
            return;
        }
        List<Application> mine = getUserApplications();
        int pending = (int) mine.stream().filter(a -> "pending".equals(a.getStatus().getCurrent())).count();
        int under = (int) mine.stream().filter(a -> "under_review".equals(a.getStatus().getCurrent())).count();
        int acc = (int) mine.stream().filter(a -> "accepted".equals(a.getStatus().getCurrent())).count();
        int rej = (int) mine.stream().filter(a -> "rejected".equals(a.getStatus().getCurrent())).count();
        TAUser.ApplicationSummary s = currentUser.getApplicationSummary();
        if (s == null) {
            s = new TAUser.ApplicationSummary();
            currentUser.setApplicationSummary(s);
        }
        s.setTotalApplications(mine.size());
        s.setPending(pending);
        s.setUnderReview(under);
        s.setAccepted(acc);
        s.setRejected(rej);
    }
    ...
    private void loadApplications() {
        ...
        syncApplicationSummaryFromApplications();
    }
