package TA_Job_Application_Module.ui;


import java.awt.Color;


public class UI_Constants {
    
    
    /** Soft-Neo accent ≈ #8C7CF0 (JobsPortalUi.ACCENT_SOFT). */
    public static final Color PRIMARY_COLOR = new Color(140, 124, 240);
    public static final Color PRIMARY_HOVER = new Color(122, 108, 214);
    
    
    public static final Color SUCCESS_COLOR = new Color(16, 185, 129);
    public static final Color WARNING_COLOR = new Color(245, 158, 11);
    public static final Color DANGER_COLOR = new Color(239, 68, 68);
    public static final Color INFO_COLOR = new Color(59, 130, 246);

    /** Offer pending — distinct from {@link #SUCCESS_COLOR} (accepted) and other status hues. */
    public static final Color OFFER_PENDING_BADGE_BG = new Color(204, 251, 241);
    public static final Color OFFER_PENDING_BADGE_FG = new Color(15, 118, 110);
    public static final Color OFFER_PENDING_BADGE_BORDER = new Color(153, 246, 228);
    public static final Color OFFER_PENDING_ACCENT = new Color(13, 148, 136);

    
    public static final Color BG_COLOR = new Color(248, 249, 250);
    public static final Color CARD_BG = Color.WHITE;
    
   
    public static final Color TEXT_PRIMARY = new Color(17, 24, 39);
    public static final Color TEXT_SECONDARY = new Color(107, 114, 128);
    
    
    public static final Color BORDER_COLOR = new Color(229, 231, 235);
    
   
    public static final Color DARK_BUTTON = new Color(26, 28, 46);
    public static final Color DARK_BUTTON_HOVER = new Color(38, 42, 62);
    
    
    public static final Color NAV_ACTIVE_BG = new Color(243, 244, 246);
}

